...
### `题库返回数据错误怎么办？`{#tk-data-error}

1.题库自身原因，可以截图发给题库作者询问解决方法

2.题库配置问题，请不要在微信/QQ中复制分享题库配置，然后再粘贴到脚本，微信/QQ可能会将部分符号识别成表情包，导致脚本无法正确解析题库配置。

### `题库 连接失败🔴 或超时怎么办？`{#tk-error}

1.网络原因，可以尝试切换手机热点

> 由于部分学校校园网可能会屏蔽题库以及脚本的请求，然后出现题目和答案解析乱码，文字为繁体字情况，然后题库也无法连接。

2.浏览器问题， 下载 OCS软件 https://docs.ocsjs.com/docs/app/，下载软件后一键初始化即可使用

> 部分浏览器脚本管理器兼容较差，下载软件版使用内置的谷歌浏览器，稳定又快速。
...
- 因为开高倍速被老师清空或者超星自动清空了、或者凌晨挂脚本被系统检测到也会清空。

### `开高倍速会怎么样`

1. 导致进度被清空，得重新看。

2. 学习通后台可以看到你开了多少倍，所以自己判断，公开课一般可以开，没人管，专业课的话你自己斟酌吧。
